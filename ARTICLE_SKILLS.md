...
本项目使用三个职责分离的文章 Skill，分别处理内容规划、技术文章写作和微信公众号与掘金平台交付。项目根目录的 `AGENTS.md` 是最高规则，Skill 的具体流程不得覆盖其中的角色、封面、日期及其他专项约束。
...
| 写微信公众号或掘金文章、平台版本或平台发布稿 | `multi-platform-article-writer` |
...
## 3. multi-platform-article-writer

### 职责

负责微信公众号与掘金场景下的完整交付：

- 将素材、博客或项目记录改写为目标平台文章
- 适配公众号手机阅读、标题、摘要、转发文案和 CTA
- 适配掘金标准 Markdown、完整代码、命令和验证步骤
- 在公众号系列中维护术语、坑点和篇目衔接
- 公众号按规则生成封面及正文配图；掘金仅在提高理解时配图
...
```text
使用 multi-platform-article-writer 把这篇技术博客改成微信公众号版本，保留核心代码，优化手机阅读节奏。
```
...
2. 用户确认后，使用 `multi-platform-article-writer` 完成公众号成稿、配图和发布前检查。

### 从技术博客到公众号版本

1. 使用 `tech-article-expert` 保证原始技术博客的深度和可验证性。
2. 使用 `multi-platform-article-writer` 将博客压缩为适合手机阅读的公众号版本。
