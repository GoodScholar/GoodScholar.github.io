...
文章类型：教程干货；目标读者：Flutter 开发者、独立开发者、技术团队负责人；核心目标：知识沉淀 + 涨粉；作者调性：独立开发者。
...
在 Flutter 项目中使用 AI 编码助手时，最常见的问题是"AI 写的代码不符合项目规范"。本文从零开始，教你如何在 Flutter 项目中搭建 Skills 体系，让 AI 编码助手自动理解并遵守你的项目架构、代码风格和最佳实践。包含完整的操作步骤、可复用的配置模板和真实坑点处理。
...
| Skill | 用途 | 触发场景 |
|-------|------|----------|
| flutter-architecture | 架构规范和分层原则 | 新建模块、修改架构 |
| flutter-code-style | 代码风格和命名约定 | 生成代码、代码审查 |
| flutter-state-management | Riverpod 使用规范 | 状态管理相关操作 |
| flutter-routing | GoRouter 路由配置 | 路由相关操作 |
